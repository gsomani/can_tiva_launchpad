#include "can0.h"	
void read_rx_msg(int msg_obj,int *id,bool *remote,int *len,char *message){
        tCANMsgObject msg;
	msg.pui8MsgData = (uint8_t*)message;
...
void can_send_message(int msg_obj,int id,bool remote,int len,char *message){
	...
	else { msg.pui8MsgData = (uint8_t*)message; CANMessageSet(CAN0_BASE, msg_obj, &msg, MSG_OBJ_TYPE_TX);}
}
